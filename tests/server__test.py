...
class FlaskServerTest(unittest.TestCase):
    ...
    def test_Add_Task_To_Database_Invalid_Input_Missing(self):
        ...
        self.assertEqual(
            response.status_code,
            422,
            'the error logic for the validator() logic in "add_tasks()" failed',
        )
    ...
    def test_Delete_Task(self):
        ...
        self.assertEqual(
            response.status_code,
            200,
            f"The Task with the ID:{task_id} does not exist! thus could not be deleted.",
        )

        tasks = Task.query.all()
        self.assertNotIn(
            test_task, tasks, f"The Task with ID:{task_id} was not deleted!"
        )
    ...
    def test_Delete_Task_Bad_Id(self):
        ...
        self.assertIsNone(
            isTask,
            "The Task with the provided ID: 00 doesnt exist. thus cant be deleted.",
        )

        self.assertEqual(
            response.status_code,
            404,
            "deletion fail! the Task with the provided ID:00 does not exist!",
        )
    ...
    def test_Amount_Of_Tasks(self):
        ...
        self.assertEqual(
            response.status_code,
            200,
            "failed to get the amount of Task entries for the current user!",
        )
    ...
    def test_Amount_Of_Tasks_Fail(self):
        ...
        self.assertEqual(
            response.status_code,
            404,
            "we arent capturing when theres no task entries in getTasksLength for the current user",
        )
    ...
    def test_Done_Tasks_Invalid_KeyPair(self):
        ...
        self.assertEqual(
            response.status_code,
            400,
            "we are no longer catching incorrect key-value pairs from done_tasks!",
        )
    ...
    def test_Create_User_Invalid_Key_Value_Input(self):
        ...
        self.assertEqual(
            response.status_code,
            422,
            "an invalid key-pair isnt being reported when creating a new account!",
        )
    ...
    def test_Create_User_invalid_key_value_empty_input(self):
        ...
        self.assertEqual(
            response.status_code,
            422,
            "An empty key-pair isnt being reported when creating a new account!",
        )
